...
\begin{document}
	...
	Sada će se na klasifikacija naglasaka u 4 tipa koja je uvedena na početku po prvi put zbilja pokazati korisnom: za svaki tip vrijede ista pravila bez obzira bio glagol \textit{ićidan} ili \textit{godan}.
	...
	Tipično prijelazno-neprijelazni parovi imaju iste tipove naglaska, izuzev glagola od 2 m\=ore (取る、取れる、折る、折れる、立つ、立てる itd.)
	
	\subsubsection*{\textit{Heiban} glagoli}
	Najčešće se miješaju s \textit{nakadaka} glagolima (i obrnuto).
	Neki od njih čak se i izgovaraju na oba načina.
	Teško je unaprijed znati koji su koji.
	...
	\begin{itemize}
		\begin{multicols}{3}
			...
			\item \pitch{す/0,る/1}（する）
			\item \pitch{や/0,る/1}（やる）
			\item \pitch{し/0,ま/1,う/1}（しまう）
		\end{multicols}
	\end{itemize}
	...
	\begin{itemize}
		\begin{multicols}{3}
			...
			\item \pitch{で/0,き/1,る/0}（できる）
			\item \pitch{く/0,だ/1,さ/1,る/0}（くださる）
		\end{multicols}
	\end{itemize}
